...
After Supabase is integrated and the model decisions this depends on are settled:
[categories](feature-categories-and-groups.md), [thumbnails](feature-thumbnail-upload.md)
and [non-Slides entries](data-non-slides-entries.md). Migrating before then means
migrating twice.
...
- **File id**: `editableUrl` is a `docs.google.com/presentation/d/<id>/edit` URL in 35
  entries — `extractFileId` in `lib/google-slides.ts` already handles it. 4 entries point
  to `drive.google.com/file/d/…` instead (see non-Slides entries).
- **Published id is not in the data.** `url` points to the old wrapper folders on the main
  site (`https://mooco.studio/Capabilities/`, …), not to Google Slides. The `2PACX-…` id has
  to be recovered from each wrapper page's embed iframe, or by re-publishing the deck from
  Slides. 9 entries have an empty `url` (AI Studio is empty on purpose, per the reference
  doc §13).
